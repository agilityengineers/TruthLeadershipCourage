...
**Marketing copy is DB-driven and overrides code defaults.** Every marketing section (hero, nav, footer…) renders from a `site_section` row; `content.ts` `resolveContent` merges the stored row *over* the registry default in `lib/site-content/src/index.ts`. So changing a default label/CTA in code does NOT change the live site once a row exists for that key (e.g. `home.hero`, `global.nav`) — you must update the stored `content` JSON (targeted `jsonb_set`, preserving sibling fields) or edit via the admin content editor. **Prod caveat:** publishing migrates schema, not content rows, so the same content edits must be re-applied to the production DB (admin editor on the live site). The api-server `dev` script builds once on start (no watch) — restart `artifacts/api-server: API Server` after any route/code change or new routes 404. Likewise, any Drizzle schema edit (new column/table) needs `pnpm --filter @workspace/db run push` or every query touching that table 500s with "column … does not exist" — even unrelated flows like login break, since session lookups select all columns.

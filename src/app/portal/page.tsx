...
export default async function PortalHome({
  ...
  const { phase: sp } = await searchParams;
        ...
        <div className="rounded-card border border-[#e6dff3] bg-[#f6f4fb] p-5">
          <div className="mb-3 flex items-center gap-2.5">
            <span className="flex h-10 w-10 items-center justify-center rounded-full bg-white">
              <Image src="/brand/wisdomtri-logo.jpg" alt="" width={30} height={30} className="h-[30px] w-[30px] object-contain" />
            </span>
            <div>
      ...
      <div className="grid items-start gap-5 lg:grid-cols-[1fr_320px]">
        <div className="flex min-w-0 flex-col gap-[18px]">
          <Card className="p-6">
            ...
            <form action={markWeekComplete}>
              <input type="hidden" name="enrollmentId" value={enr.id} />
              <input type="hidden" name="weekNo" value={week} />
              <Button type="submit">Mark week complete</Button>
            </form>
          </Card>
        ...
        <Card className="p-[22px]">
          ...
          <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
            {libraryTiles.map((t) => (
              ...
              >
                <Image src={`/brand/${t.icon}`} alt="" width={34} height={34} className="h-[34px] w-[34px] shrink-0" />
                <div>
...
async function markWeekComplete(formData: FormData) {
  "use server";
  const { markWeekCompleteAction } = await import("@/server/progress-actions");
  await markWeekCompleteAction(
    String(formData.get("enrollmentId")),
    Number(formData.get("weekNo")),
  );
}

...
import { useListCompanyPeople } from "@workspace/api-client-react";
...
export default function CompanyPeoplePage() {
  ...
  const { data } = useListCompanyPeople();
  const people = data ?? [];
            ...
            <Card key={p.id} className="p-5">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <div className="text-[12px] text-muted-2">Participant</div>
                </div>
              </div>
              <div className="mt-4">
                <div className="mb-1 flex justify-between text-[12px] text-muted-2">
                  <span>{enr?.cohortName ?? "Not enrolled"}</span>
                  <span>{pct}%</span>
                </div>
